...
class _ProfileWidgetState extends State<ProfileWidget> {
  ...
  Future<void> _fetchAccountHolderName() async {
    ...
    final url = Uri.parse(
        'http://127.0.0.1:8000/api/profilee/account-holder/?token=$token');
    try {
  ...
  Future<void> _fetchAccountNumber() async {
    ...
    final url = Uri.parse(
        'http://127.0.0.1:8000/api/profilee/account-number/?token=$token');
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              final url = Uri.parse('http://127.0.0.1:8000/api/profilee/change-password/');
              try {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              final url = Uri.parse('http://127.0.0.1:8000/api/profilee/change-pin/');
              try {
                final response = await http.post(
                  url,
                  headers: {
